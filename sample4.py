...
# ダックタイピング
# この方式では、オブジェクトの型（クラス）を判断するのではなく、オブジェクトが必要なメソッドやプロパティを持っていることを確認することで、そのオブジェクトが適切な処理を実行できるかどうかを判断します

#抽象クラス
import abc
#抽象クラスであるということをabcを使用して表す。
class Person(metaclass=abc.ABCMeta):
    def __init__(self, age=1):
        self.age = age
    #継承先でdriveというメソッドを実装してもらうようにする。
    #実装してほしいものを定義する。
    #あまり作らない
    @abc.abstractmethod
    def drive(self):
